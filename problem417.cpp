...
private:
	vector<pair<int, int>> pacificAtlantic(vector<vector<int>>& matrix) {
		...
		for (int i = 0; i<row; i++)
		{
			...
			flow(Atlantic, matrix, i, column - 1);
		}
		for (int i = 0; i<column; i++)
		{
			flow(Pacific, matrix, 0, i);
			flow(Atlantic, matrix, row - 1, i);
		}
		for (int i = 0; i<row; i++)
		{
			for (int j = 0; j<column; j++)
			{
				if (Pacific[i][j] && Pacific[i][j] == Atlantic[i][j])
					res.push_back(make_pair(i, j));
			}
		}
		return res;
	}
	void flow(vector<vector<bool>>& ocean, vector<vector<int>>& matrix, int x, int y)
	{
		ocean[x][y] = true;
		...
		while (!q.empty())
		{
			...
			q.pop();
			if (a - 1 >= 0 && matrix[a - 1][b] >= matrix[a][b] && ocean[a - 1][b] != true)
			{
				ocean[a - 1][b] = true;
				q.push(make_pair(a - 1, b));
			}
			if (a + 1 < matrix.size() && matrix[a + 1][b] >= matrix[a][b] && ocean[a + 1][b] != true)
			{
				ocean[a + 1][b] = true;
				q.push(make_pair(a + 1, b));
			}
			if (b - 1 >= 0 && matrix[a][b - 1] >= matrix[a][b] && ocean[a][b - 1] != true)
			{
				ocean[a][b - 1] = true;
				q.push(make_pair(a, b - 1));
			}
			if (b + 1 < matrix[0].size() && matrix[a][b + 1] >= matrix[a][b] && ocean[a][b + 1] != true)
			{
				ocean[a][b + 1] = true;
				q.push(make_pair(a, b + 1));
			}
